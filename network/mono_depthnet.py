...
class MonoDepthNet(nn.Module):
    ...
    def forward(self, input_images, intrinsics, extrinsics, depth=None, attention=False):
        depth_feature = self.depth_encoder(input_images)
        outputs = self.depth_decoder(depth_feature, intrinsics, extrinsics, depth, attention, org_img_size=input_images.shape[2:])
        return outputs
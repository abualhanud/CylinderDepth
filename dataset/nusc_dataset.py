...
class NUSCdataset(torch.utils.data.Dataset):
    ...
    def __init__(self, cfg,mode,**kwargs):
        ...
        self.data_root = '/home/abualhanud/data/abualhanud/full_data'
        self.depth_path='/home/abualhanud/data/abualhanud/full/depth_lidar_gt_surround'
        self.overlap_depth_path='/home/abualhanud/data/abualhanud/full/overlap_depth'
    ...
    def __getitem__(self, idx):
        ...
        for index_spatial,cam in enumerate(self.alias_cams):
            ...
            if self.with_depth:
                ...
                if self.cfg['eval']['overlap'] is True:
                    data.update({
                        'depth':np.load(os.path.join(self.overlap_depth_path, 'samples', rgb_filename.split('samples/', 1)[1]).replace('.jpg', '.npy'))[None,:] 
                    })
                else:
                    data.update({
                        'depth':np.load(os.path.join(self.depth_path, 'samples', rgb_filename.split('samples/', 1)[1]).replace('.jpg', '.npy'))[None,:] #depth_lidar_gt_surround/samples #overlap_depth/samples
                    })
            # if depth is returned

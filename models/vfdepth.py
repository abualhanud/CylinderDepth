...
class VFDepthAlgo(BaseModel):
    ...
    def estimate_vfdepth(self, inputs):
        ...
        if self.mode !='train':
            ...
            depths_attn = []
            for i in range(6):  # cams 0–5
                d = outputs[('cam', i)][('depth', 0)]  # shape: [B, 1, 384, 640]
                depths_attn.append(d.unsqueeze(1))

            # stack and reshape
            depths_attn = torch.cat(depths_attn, dim=1)  # shape: [B, 6, 384, 640]
            B, N, C, H, W = depths_attn.shape
            last_dim = self.models['depth_net'].depth_encoder.last_dim
            depth_attn = F.interpolate(depths_attn.view(B * N, C, H, W),size=(int(H/last_dim), int(W/last_dim)), mode='bilinear', align_corners=False)
            depths_attn = depth_attn.view(B, N, C, int(H/last_dim), int(W/last_dim))
            depth_feats = self.predict_depth(inputs, depth=depths_attn, attention=True)
            for cam in range(self.num_cams):
                outputs[('cam', cam)].update(depth_feats[('cam', cam)])
            self.compute_depth_maps(inputs, outputs)
            return outputs
    ...
    def predict_depth(self, inputs, depth=None, attention=False):
        """
        ...
        disps = net(input_depth.reshape(B*N,C,H,W), intrinsics=inputs[('K', 0)][:, :, :3, :3].reshape(B*N,3,3),
                    extrinsics=inputs['extrinsics'].reshape(B*N,4,4), depth=depth, attention=attention)
